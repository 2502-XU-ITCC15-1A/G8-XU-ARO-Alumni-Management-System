...
export default function ApplicationReview() {
  ...
  const openModal = (app) => {
    ...
    setUniversityIdNumber(
      app.alumniProfile?.universityIdNumber ||
        app.universityIdNumber ||
        ''
    );
  };
  ...
  const handleAction = async (id, status) => {
    ...
    if (status === 'approved' && !universityIdNumber.trim()) {
      setValidationError(
        'XU University ID Number is required for approval.'
      );
      return;
    }
...
const counts = {
  ...
  const STAT_CARDS = [
    ...
    },
    {
      ...
      cls: 'app-stat-pending',
      valClass: 'text-warning',
    },
    {
      label: 'Approved',
      value: counts.approved,
      cls: 'app-stat-approved',
      valClass: 'text-success',
    },
    {
      label: 'Rejected',
      value: counts.rejected,
      cls: 'app-stat-rejected',
  ...
  const formatEducation = (app) => {
    ...
    return [...app.education]
      ...
      .map((e, index) => {
        ...
        const subTitle = e.degree
          ? `${e.level} • Graduated ${
              e.yearGraduated || '—'
            }`
          : e.yearGraduated
          ? `Graduated ${e.yearGraduated}`
          : '';

        return (
          <div
            key={index}
            className="mb-2 pb-2 border-bottom border-light style-education-item"
          >
            <div
  ...
  return (
    <div className="p-4 p-lg-5">
      ...
      <p
        ...
        style={{ fontSize: 14 }}
      >
        Verify alumni information and approve or reject ID
        applications
      </p>
        ...
        {STAT_CARDS.map((card) => (
          ...
          >
            <div className={`app-stat-card ${card.cls}`}>
              <div
                className={`small mb-1 ${card.valClass}`}
              >
                {card.label}
              </div>

              <div
                className={`fw-bold fs-4 ${card.valClass}`}
              >
                {loading ? '—' : card.value}
              </div>
            </div>
            ...
            <>
              <div className="table-responsive">
                ...
                >
                  <thead>
                    <tr>
                      <th className="fw-semibold text-dark border-top-0 ps-2">
                        Name
                      </th>

                      <th className="fw-semibold text-dark border-top-0 text-end">
                        Applied Date
                      </th>
                      ...
                      <th className="fw-semibold text-dark border-top-0 text-end pe-3">
                        Actions
                      </th>
                    </tr>
                  </thead>
                  ...
                  <tbody>
                    {paginatedApps.map((app) => (
                      <tr key={app._id}>
                        <td className="text-primary fw-medium ps-2">
                          {getFullName(app)}
                        </td>
