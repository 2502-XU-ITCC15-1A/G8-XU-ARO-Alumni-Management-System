      ...
      >
        ...
        >
          ...
          <div className="p-4">
            ...
            <div
              className="alert alert-warning d-flex align-items-start gap-2 mb-0"
              style={{
                fontSize: 13,
              }}
            >
...
export default function ApprovedApplications() {
  ...
  const [confirmApp, setConfirmApp] =
    useState(null);
  ...
  const handleDelete =
    async () => {
      ...
      try {
        await axios.delete(
          `/api/IdApplication/${confirmDelete._id}`
        );
  ...
  const handleConfirmPayment =
    async () => {
      ...
      try {
        await axios.put(
          `/api/bookcenter/${confirmApp._id}/verify-payment`
        );
  ...
  const counts = {
    ...
    awaiting: apps.filter(
      (a) =>
        !a.paymentVerified &&
        a.status === 'approved'
    ).length,

    printing: apps.filter(
      (a) =>
        a.status === 'printing'
    ).length,

    released: apps.filter(
      (a) =>
        a.status === 'released'
    ).length,
  };
  ...
  return (
    <div className="p-4">
      ...
      <p
        ...
        style={{ fontSize: 13 }}
      >
        Alumni ID applications
        approved by XU-ARO —
        confirm payment when
        alumni pays at the Book
        Center
      </p>
                      ...
                      <td>
                        <div className="d-flex align-items-center gap-1">
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `/external-portal/applications/${app._id}`
                              )
                            }
                ...
                >
                  ...
                  <strong>
                    {confirmDelete
                      .userId
                      ?.name ||
                      confirmDelete.universityIdNumber}
                  </strong>
                  . This action
                  cannot be
                  undone.
